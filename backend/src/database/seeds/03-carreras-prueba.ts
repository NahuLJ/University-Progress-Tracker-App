...
const CARRERAS: {
...
}[] = [
  {
    ...
    descripcion:
      'Carrera de grado orientada al desarrollo de software y sistemas.',
    duracionAnios: 5.5,
    materias: [
      {
        codigo: 'ASIS1',
  ...
  },
  {
    ...
    descripcion:
      'Carrera orientada a la gestión y dirección de organizaciones.',
    duracionAnios: 4.5,
    materias: [
      {
        codigo: 'ADM1',
...
export async function seedCarrerasPrueba(dataSource: DataSource) {
  ...
  for (const c of CARRERAS) {
    ...
    console.log(
      `  ✓ Carrera creada: ${c.nombre} (${c.materias.length} materias)`,
    );
  }
  ...
  for (const c of CARRERAS) {
    ...
    for (const plan of planes) {
      const existeProgreso = await progRepo.findOne({
        where: {
          usuarioCarrera: { usuarioCarreraId: uc.usuarioCarreraId },
          materia: { materiaId: plan.materia.materiaId },
        },
      });
